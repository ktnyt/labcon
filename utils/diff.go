...
func mustMarshalStringYAML(a interface{}) string {
	p, err := yaml.MarshalWithOptions(a, yaml.Indent(2))
	if err != nil {
		panic(err)
	}
	return string(p)
...
// ObjDiff returns the line diff of two objects serialized by YAML.
func ObjDiff(a, b interface{}) []Op {
	sa := mustMarshalStringYAML(a)
	sb := mustMarshalStringYAML(b)
	return LineDiff(sa, sb)
}
...
// ReaderDiff returns the line diff of two io.Reader contents.
func ReaderDiff(a, b io.Reader) []Op {
	sa := readString(a)
	sb := readString(b)
	return LineDiff(sa, sb)
}

// JoinOps joins the ops with the given delimiter.
func JoinOps(ops []Op, delim string) string {
